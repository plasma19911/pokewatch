...
let items = [], generated = null, offline = false;
let fRegion = 'all', fCat = 'all';
let lastSeen = 0;
...
function chips(host, map, counts, active, onPick) {
  host.innerHTML = '';
  ...
  Object.keys(map).forEach(k => {
    ...
    add(k, Array.isArray(map[k]) ? map[k][0] : map[k], counts[k]);
  });
}
...
function render() {
  ...
  chips($('regionChips'), REGIONS, byRegion, () => fRegion, k => fRegion = k);
  ...
  const list = $('list');
  if (!shown.length) {
    list.innerHTML = `<div class="note"><b>Nichts hier</b>
      Diese Kombination aus Herkunft und Vorfall ist leer.
      Tipp einen der Filter auf „Alle".</div>`;
    return;
  }
...
function readSeed() {
  ...
  try {
    ...
    items = d.items || []; generated = d.generated || null;
    ...
    const timer = setInterval(async () => {
      ...
      try {
        ...
        if (d.generated && d.generated !== before) {
          clearInterval(timer);
          items = d.items || []; generated = d.generated;
          btn.disabled = false; btn.classList.remove('spin');
          setStatus(); render();
        }
      } catch (e) {}
